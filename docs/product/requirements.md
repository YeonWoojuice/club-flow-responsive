...
- 이름, 이메일, 연락처, 학번, 지원 당시 학년으로 수동 등록
...
- 동아리 이름, 지원자 이름, 카카오톡 링크를 메일 템플릿 변수로 사용
...
- 학기별 부원 기록 조회
- 선택한 학기의 부원만 조회
- 부원 목록은 회비 납부 여부로 재정렬하거나 제외하지 않고 기본적으로 학년순으로 표시
- 부원 목록의 회비 상태는 읽기 전용으로 표시하고 실제 변경은 회비 관리에서 처리
- 학기별 단일 회비 금액과 납부 기한 설정
- 학기별 날짜 구간·환불 비율 설정과 원 미만 환불액 버림
- 부원별 회비 부과·납부·면제·환불 기록 및 학기 합계 확인
- 탈퇴 상태 변경 이력의 실제 탈퇴일을 기준으로 환불액 계산
- 부원별 카카오톡·디스코드 초대 완료 여부 확인
- 부원 상태를 회원·준회원·비활동으로 변경하고, 비활동 부원을 탈퇴 처리
- 부원 상태 변경 사유·변경자·변경 시간 이력 조회
- 종료된 이전 학기의 잔류 부원을 활성 새 학기로 이월
- 표 파일(CSV·엑셀), Google Form 응답 Sheet, 일반 Google Sheet에서 이월 대상 확인
- 지원자 등록과 잔류 이월에서 해당 학기의 학년을 필수로 입력
...
- 부원 상태는 회원(`REGULAR`), 준회원(`ASSOCIATE`), 비활동(`INACTIVE`), 탈퇴(`WITHDRAWN`)로 구분한다.
- 회원은 모든 동아리 활동에 필수적으로 참여하고, 준회원은 개인 사정으로 일부 활동에만 참여하며 멘토링 등의 방식으로 기여한다.
- `REGULAR ↔ ASSOCIATE`, `REGULAR ↔ INACTIVE`, `ASSOCIATE ↔ INACTIVE`, `INACTIVE → WITHDRAWN`으로 변경할 수 있다.
- 탈퇴는 비활동 상태에서만 처리하며, `WITHDRAWN`은 되돌릴 수 없고 탈퇴 사유 입력과 최종 확인이 필요하다.
